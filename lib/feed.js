...
exports.article = function(item, count) {
	...
	"description.__text": item.description, // html
...
exports.article_match = function(article, opts) {
	...
	let arr = [article.title,
		   ...
		   html2text(article["description.__text"]),
...
exports.Grep = class extends Transform {
	...
	let expect = !opts.v  // see fp 'readable' event handler below
	let tr = this

	this.first_bytes_are_here = false
	this.header()
	this.fp = new FeedParser({ addmeta: false })

	this.fp.on('meta', function() {
	    tr.first_bytes_are_here = true
	    let obj = exports.meta2arr(this.meta)
	    tr.push(tr.meta(obj))
	})
	...
	this.fp.on('readable', function() {
	    ...
	    while ((raw_item = stream.read())) {
		let item = exports.article(raw_item, ++articles_counter)

		if (opts.n && tr.articles_matched >= opts.n) {
//		    console.error('-n', tr.articles_matched)
		    tr.end()	// FIXME
		    break
		}

		// by default, `expect` is true, but it's false with -v CLO
		if (exports.article_match(item, opts) === expect) {
		    tr.articles_matched++
		    tr.push(tr.article(item))
		}
	    }
	})

	this.fp.on('end', function (err) {
	    if (err) tr.handle_error(err)
	})
    }

    header() {}
    footer() {}
    handle_error(_err) { throw new Error('implement me!') }
