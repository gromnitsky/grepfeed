...
exports.Grep = class extends Transform {
	...
	let expect = !this.opts.v  // see fp 'readable' event handler below
	let tr = this
	...
	this.fp.on('readable', function() {
		...
		if (tr.opts.n && tr.articles_matched >= tr.opts.n) {
//		    console.error('-n', tr.articles_matched)
		...
		if (exports.article_match(item, tr.opts) === expect) {
		    tr.articles_matched++
    ...
    _flush(done) {
	this.fp.end() // w/o this I can't catch "not a feed" errors
	this.footer()
	done()
    }
